...
| 6 — Card/cash extensions | Card preferences, payment-intent lifecycle, editable calendar, disputes, deterministic activity centre; cash product and ATM transfer | Turn verified card data into safe actions and prevent cash double counting | Utilisation guardrail, explicit rewards, correctable/removable reminders, cancellable payment intent, explicit idempotent manual transfer recording, disputes; ATM moves bank→cash and later cash purchase is spend | Card service/UI tests, transfer invariants, mobile Card workspace |
