...
class CardCalendarEvent(Base):
    ...
    source_kind: Mapped[str] = mapped_column(String(24), nullable=False, default="manual")
    source_label: Mapped[str] = mapped_column(String(160), nullable=False, default="User-entered")
    source_identifier: Mapped[str | None] = mapped_column(String(128), nullable=True)
    annual_fee_amount: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True)
    fee_reversal_condition: Mapped[str | None] = mapped_column(String(500), nullable=True)
    fee_reversal_status: Mapped[str | None] = mapped_column(String(24), nullable=True)
    milestone_spend_target: Mapped[Decimal | None] = mapped_column(Numeric(18, 2), nullable=True)
    milestone_period_start: Mapped[date | None] = mapped_column(Date, nullable=True)
    milestone_period_end: Mapped[date | None] = mapped_column(Date, nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=lambda: datetime.now(UTC)
    )
