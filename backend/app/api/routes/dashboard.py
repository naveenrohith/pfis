...
from app.schemas.dashboard import MonthlySnapshotResponse, WorkspaceResponse
...
@router.get("/monthly-snapshot", response_model=MonthlySnapshotResponse)
async def get_monthly_snapshot(
    ...
    """Return a deterministic month summary composed from existing read models."""
    user_id = resolve_user_scope(user_id, current_user)
    return await WorkspaceService(db).get_monthly_snapshot(user_id, month, year)
